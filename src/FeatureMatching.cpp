					 ...
					 double& mindist)
{
	...
	switch (method)
	{
		case Surf:
		{
			...
			Ptr<SurfFeatureDetector> detector = SURF::create(100,3,2);
			detector->detect(img_1, keypts1);
			detector->detect(img_2, keypts2);
			Ptr<SurfDescriptorExtractor> extractor = SURF::create(100,3,2);
			extractor->compute(img_1, keypts1, descriptors_1);
			extractor->compute(img_2, keypts2, descriptors_2);

			Ptr<DescriptorMatcher> matcher  = DescriptorMatcher::create ( "BruteForce-L2" );
...

	{
        ...
        cout<<"算法用时："<<time_used.count()<<" 秒。"<<endl;
		fliter_matches.clear();
		fliter_matches = b_matches;

        std::vector< DMatch > t_matches;
        set<int> existing_trainIdx;

        double max_dist = 0;
        double min_dist = 1000.0;

        for(unsigned int i = 0; i < b_matches.size(); i++ )
        {
            double dist = b_matches[i].distance;
            if( dist < min_dist ) min_dist = dist;
            if( dist > max_dist ) max_dist = dist;
        }
        vector< DMatch > good_matches,temp_matches;
        ...
        for(unsigned int i = 0; i < b_matches.size(); i++ )
        {
            ...
            if(b_matches[i].distance > 0.0 &&b_matches[i].distance < 15*min_dist)
            {
                //如果一个描述符在train中只出现一次，说明它没有其他的重匹配，
                if (existing_trainIdx.find(b_matches[i].trainIdx) == existing_trainIdx.end()) {
                    t_matches.push_back(b_matches[i]);
                    existing_trainIdx.insert(b_matches[i].trainIdx);
                }

            }
        }
        fliter_matches = t_matches;
		sort(fliter_matches.begin(),fliter_matches.end(),cl_greater);
		mindist = fliter_matches[(int)fliter_matches.size()/2].distance;
	}
